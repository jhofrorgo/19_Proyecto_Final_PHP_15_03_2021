<?php
    ...
    class Service_perfil extends Service_index implements Mensajes,Plantilla {
        static $servicePerfil; 
        public $plantilla; 
        static public function getInstance(){
            if(!(self::$servicePerfil instanceof self)){
                self::$servicePerfil = new Service_perfil();
            }
            return self::$servicePerfil; 
        }
        public function SectionPerfil(){
        $this->plantilla['Section'] = "";
        $this->plantilla['Imagenes'] = "";
        foreach ($this::service['sectionPerfil']["Descripcion"] as $key => $value) {
            switch ($key) {
                case 'Titulo':
...
FIN;
        ...
        foreach ($this::service['sectionPerfil']["Imagenes"] as $key => $value) {
            if($key!=0){
